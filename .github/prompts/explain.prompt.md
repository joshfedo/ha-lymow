...
Explain `${input:args}` clearly.

## Mode

If `${input:args}` includes the word `verbose` (for example, `/explain my-function verbose`), produce all five sections below. Strip the word `verbose` from the target name when interpreting what to explain.
