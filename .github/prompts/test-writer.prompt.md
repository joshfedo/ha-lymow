...
- **One behavior per test**. Multiple related asserts on the same observed output are fine (this is established style in this repo's pytest suite). If a test name needs "and", split it into two tests — that signals two behaviors, not two assertions.
