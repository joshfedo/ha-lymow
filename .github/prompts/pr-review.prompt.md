...
Check `${input:args}` for the word `verbose`. Strip it from the argument string before parsing the rest.
...
Parse `${input:args}` to determine what to review:

- **PR number** (e.g., `123` or `#123`): fetch with `gh pr view ${input:args}`. This is the full PR review path (includes PR quality checks in Step 2).
