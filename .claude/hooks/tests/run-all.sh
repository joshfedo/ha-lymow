...
#   }
#
# Exit 0 on all pass, 1 on any test failure, 2 on a setup failure (e.g. `jq` missing).
...
run_case() {
  ...
  # Portable mktemp: BSD/macOS mktemp without a template fails, so supply one.
  out_file=$(mktemp "${TMPDIR:-/tmp}/dotclaude.XXXXXX")
  err_file=$(mktemp "${TMPDIR:-/tmp}/dotclaude.XXXXXX")

  # Build env-injection argv directly from the fixture's `.env` object so
  # values can safely contain spaces, quotes, or shell-special characters.
  # Each entry of `env_argv` is one `KEY=value` token, passed as a separate
  # positional arg to `env`. Word-splitting the entries via `env $vars` would
  # break for values like `"a b c"` or `KEY=$dangerous`.
  local env_argv=()
  while IFS= read -r kv; do
    [[ -n "$kv" ]] && env_argv+=("$kv")
  done < <(jq -r '(.env // {}) | to_entries[] | "\(.key)=\(.value|tostring)"' "$fixture")

  if [[ ${#env_argv[@]} -gt 0 ]]; then
    printf '%s' "$stdin" | env "${env_argv[@]}" bash "$hook_path" >"$out_file" 2>"$err_file"
  else
