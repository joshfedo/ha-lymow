...
# Use shell parameter expansion for path splitting — consistent with
# protect-files.sh and block-build-artifacts.sh, and avoids the BSD/macOS
# `basename`/`dirname` `--` portability quirks.
BASENAME=${FILE_PATH##*/}
EXTENSION="${BASENAME##*.}"
NAME="${BASENAME%.*}"
DIR=${FILE_PATH%/*}
