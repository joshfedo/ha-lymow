"""Throwaway: send a single userCtrl from CLI. Usage: uv run python scripts/_one_off.py <userCtrl>"""
...
async def main(code: int) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with aiomqtt.Client(
            hostname=iot_host,
            port=443,
            identifier=f"lymow-oneoff-{uuid.uuid4().hex[:8]}",
            ...
            print(f"send userCtrl={code} hex={pb.hex()}")
